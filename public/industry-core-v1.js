...
function enhanceModal(modal){
 ...
 const ai=window.BOLSRivalProductionAI?.dashboard?.(name);if(ai){const pipe=document.createElement('section');pipe.className='industryAIPipeline';const ps=ai.projects||[];const stageLabel={development:'DEVELOPMENT', 'pre-production':'PRE-PRODUCTION',production:'IN PRODUCTION','post-production':'POST-PRODUCTION',scheduled:'MARKETING / SCHEDULED'};pipe.innerHTML=`<div class="industryCoreKicker">🤖 STUDIO PRODUCTION PIPELINE · AI CONTROLLED</div><div class="industryAIPipelineGrid">${ps.slice().sort((a,b)=>String(a.status).localeCompare(String(b.status))).slice(0,8).map(p=>`<article class="industryAIProject"><div class="industryAIProjectTop"><span>${esc(stageLabel[p.status]||String(p.status||'PROJECT').toUpperCase())}</span><b>${esc(p.scale||'wide').toUpperCase()}</b></div><h4>${esc(p.title)}</h4><p>${esc(p.genre)} · ${fmt(num(p.budget))} budget</p><div class="industryAIProjectMeta"><span>CREATED W${Math.max(1,((num(p.createdAbs,1)-1)%52)+1)}</span><span>RELEASE TARGET W${Math.max(1,(num(p.targetReleaseAbs,1)%52)+1)}</span></div></article>`).join('')||'<div class="industryAIPipelineEmpty"><b>No active productions.</b><span>The studio will evaluate new projects as the simulation advances.</span></div>'}</div><div class="industryCoreSub"><b>THIS STUDIO IS ACTUALLY PRODUCING</b><span>Projects are created, reviewed, greenlit, produced, moved through post, scheduled and released by the rival studio AI rather than appearing on a fixed weekly calendar.</span></div>`;const metrics=modal.querySelector('.rivalStudioMetrics');if(metrics)metrics.after(ledger,pipe);else h.after(ledger,pipe);}else{const metrics=modal.querySelector('.rivalStudioMetrics');if(metrics)metrics.after(ledger);else h.after(ledger);}
