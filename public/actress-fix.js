...
(() => {
  ...
  const observer=new MutationObserver(()=>{
    ...
    actresses.slice(0,30).forEach(o=>{const x=o.x;const b=document.createElement('button');b.className='talentRow';b.dataset.talent=String(o.i);b.dataset.talentRole='Actress';const s=window.__BOL_STATE__||{};const rel=s.relationships?.[x.name];const score=rel&&Number.isFinite(Number(rel.score))?Math.max(0,Math.min(100,Number(rel.score))):null;const tone=score===null?'none':score>=70?'good':score<=40?'bad':'neutral';const label=score===null?'NO RELATIONSHIP':score>=90?'TRUSTED PARTNER':score>=75?'STRONG':score>=60?'POSITIVE':score>=40?'NEUTRAL':score>=20?'COLD':'HOSTILE';b.innerHTML=`<span class="talentAvatar">${esc(x.name[0])}</span><div><em class="talentRoleLabel">🎭 ACTRESS</em><b>${esc(x.name)}</b><div class="crewRelationshipMeter ${tone}" title="Studio relationship with ${esc(x.name)}"><div class="crewRelationshipTop"><span>RELATIONSHIP</span><b>${score===null?'—':`${score}/100`}</b></div><div class="crewRelationshipTrack"><i style="width:${score===null?0:score}%"></i></div><small>${label}</small></div><small>Actress · ⭐ ${x.talent} · $${(x.fee/1000000).toFixed(1)}M asking</small></div><strong>VIEW →</strong>`;list.appendChild(b);});
  });
