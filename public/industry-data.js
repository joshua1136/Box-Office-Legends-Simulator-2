...
/* Rival / platform logo placeholders. These are intentionally logo-shaped placeholders, not copied trademark artwork. Replace the SVG/text marks with licensed or supplied assets later without changing the game data. */
window.industryLogoPlaceholder=function(name,kind='studio'){if(!document.getElementById('industryLogoPlaceholderCss')){const css=document.createElement('style');css.id='industryLogoPlaceholderCss';css.textContent=`.industryLogoPlaceholder{width:46px;height:46px;flex:0 0 46px;border:1px solid #5a4828;border-radius:12px;display:grid;place-items:center;background:radial-gradient(circle at 35% 25%,#5a4521,#15120d 68%);box-shadow:0 0 18px #c49a3a14,inset 0 0 14px #0008;color:#d7b35e;position:relative;overflow:hidden}.industryLogoPlaceholder:after{content:'LOGO';position:absolute;bottom:3px;left:0;right:0;text-align:center;font-size:4px;letter-spacing:1px;color:#75633e;opacity:.8}.industryLogoPlaceholder b{font:800 13px Georgia,serif;letter-spacing:-.4px;text-shadow:0 0 10px #d4aa5640}.industryLogoPlaceholder.warner{border-radius:50%;background:radial-gradient(circle,#c5a45d 0 7%,#1b1710 8% 62%,#6b542d 63% 66%,#15120d 67%)}.industryLogoPlaceholder.universal{border-radius:50%;}.industryLogoPlaceholder.universal b{font-size:24px}.industryLogoPlaceholder.disney{border-radius:50%;}.industryLogoPlaceholder.disney b{font-size:22px}.industryLogoPlaceholder.netflix{border-radius:10px;background:linear-gradient(145deg,#3a1717,#160d0d);color:#d9a4a4}.industryLogoPlaceholder.netflix b{font-size:25px;font-family:Arial,sans-serif}.industryLogoPlaceholder.hbo{border-radius:8px;background:#181522;color:#c4a8df}.industryLogoPlaceholder.hbo b{font-family:Arial,sans-serif;font-size:11px;letter-spacing:1px}.industryLogoPlaceholder.sony b,.industryLogoPlaceholder.amazon b{font-family:Arial,sans-serif;font-size:8px;letter-spacing:.5px}.industryLogoPlaceholder.apple b{font-family:Arial,sans-serif;font-size:20px}`;document.head.appendChild(css)}const n=String(name||'').toLowerCase();let key='generic',mark=String(name||'?').slice(0,2).toUpperCase(),label=name||'Studio';if(/warner/.test(n)){key='warner';mark='WB';label='Warner Bros.'}else if(/universal/.test(n)){key='universal';mark='U';label='Universal'}else if(/disney/.test(n)){key='disney';mark='✦';label='Disney'}else if(/netflix/.test(n)){key='netflix';mark='N';label='Netflix'}else if(/max|hbo/.test(n)){key='hbo';mark='HBO';label='HBO'}else if(/sony/.test(n)){key='sony';mark='SONY';label='Sony'}else if(/paramount/.test(n)){key='paramount';mark='★';label='Paramount'}else if(/lionsgate/.test(n)){key='lionsgate';mark='LG';label='Lionsgate'}else if(/a24/.test(n)){key='a24';mark='A24';label='A24'}else if(/amazon|prime/.test(n)){key='amazon';mark='AMZ';label='Amazon'}else if(/apple/.test(n)){key='apple';mark='';label='Apple'}const cls=`industryLogoPlaceholder ${key} ${kind}`;return `<span class="${cls}" aria-label="${String(label).replace(/[&<>\"]/g,'') } logo placeholder"><b>${mark}</b></span>`};
