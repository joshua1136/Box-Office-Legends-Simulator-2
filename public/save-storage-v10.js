...
(()=>{
  ...
  async function clearSlot(slot){const n=Math.max(1,Math.min(SLOTS,Number(slot)||1));try{const db=await open();await new Promise((resolve,reject)=>{const t=db.transaction(STORE,'readwrite'),st=t.objectStore(STORE);const get=st.get(`slot:${n}`);let studio='';get.onsuccess=()=>{studio=String(get.result?.studio||'');st.delete(`slot:${n}`);if(studio)st.delete(`auto:${studio}`)};get.onerror=()=>{st.delete(`slot:${n}`)};t.oncomplete=resolve;t.onerror=()=>reject(t.error)});db.close();}catch{}try{const a=legacyRead();a[n-1]=null;localStorage.setItem(LEGACY,JSON.stringify(a));}catch{}try{const auto=JSON.parse(localStorage.getItem(AUTO)||'null');if(Number(auto?.state?.saveSlot||auto?.state?.slot)===n)localStorage.removeItem(AUTO)}catch{}return true}
