...
function upcomingFilms(s,name){const c=s?.talentContracts?.[name];return c?[{title:c.title,filmId:c.filmId,endWeek:c.endWeek}]:[]}
...
function setup(){document.addEventListener('click',ev=>{const btn=ev.target?.closest?.('#executeContract');if(!btn)return;const el=btn.closest('.talentContractModal');if(!el)return;const c=readContract(el);if(!c.film)return;const chance=acceptance(c);if(Math.random()*100>chance){ev.preventDefault();ev.stopImmediatePropagation();const films=upcomingFilms(state(),c.name);notifyDecline(c.name,talentText(c.name,c.movies,films),films,c.offer,c.ask)}},true);const mo=new MutationObserver(()=>{document.querySelectorAll('.talentContractModal').forEach(attachEconomics);injectRoster()});mo.observe(document.body,{childList:true,subtree:true});window.BOLTalentContractsV2={version:2,processWeekly,renderContracts,acceptance}}
setup();
})();