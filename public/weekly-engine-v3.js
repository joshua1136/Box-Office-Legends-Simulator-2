...
(function(){
 ...
 function finish(t){if(!t?.sourceState||!t?.nextState)return false;const actual=t.sourceState,next=t.nextState,c=calendar(t.from);copyInto(actual,next);actual.year=c.year;actual.week=c.week;actual.__weeklyTransitionId=t.id;actual.__weeklyPrevious={...t.from};safe(()=>{actual.energy=typeof energyCapacity==='function'?energyCapacity(actual):100});if(typeof isTestStudio==='function'&&isTestStudio(actual)&&typeof TEST_MONEY!=='undefined')actual.money=TEST_MONEY;window.__BOL_STATE__=actual;window.__BOL_CURRENT_STATE__=actual;document.querySelectorAll('.studioReportModal,.sr2-backdrop,.weeklyModal,.nextWeekLoading').forEach(x=>x.remove());safe(()=>window.saveCurrent?.(actual,true));safe(()=>window.BOLS2Core?.checkpoint?.('weekly-autosave'));safe(()=>window.start?.(actual));busy=false;tx=null;return true}
 function simulate(s){if(busy)return false;const source=clone(s||live());if(!source)return false;busy=true;const id='weekly-'+Date.now()+'-'+Math.random().toString(36).slice(2);const from={year:+source.year||1,week:+source.week||1};let next=clone(source);safe(()=>{if(typeof simulateWeeklyReportLegacy==='function')simulateWeeklyReportLegacy(next)});document.querySelectorAll('.weeklyModal,.nextWeekLoading').forEach(x=>x.remove());const report=typeof window.BOLS2StudioReport?.render==='function'?window.BOLS2StudioReport.render(next,from.week,0):null;tx={id,sourceState:s||live(),nextState:next,from,reportReady:!!report};busy=false;if(!report){tx=null;safe(()=>window.toast?.('Weekly report could not be opened. Please try NEXT WEEK again.'));return false}return true}
