...
const coach=(state)=>{
 ...
 const advance=()=>{if(idx===2){document.querySelector('.projectBtn')?.click();idx=3;setTimeout(render,450);return}if(idx===4){document.querySelector('#next')?.click();idx=5;setTimeout(render,900);return}if(idx===5){document.querySelector('#continueStudioReport')?.click();idx=6;setTimeout(render,650);return}if(idx<steps.length-1){idx++;render();}else{set({version:1,completed:true,studio:state.studioName,completedAt:new Date().toISOString()});remove();state.onboarding={version:1,complete:true};try{window.saveCurrent?.(state,true)}catch{}window.start?.(state);}};
